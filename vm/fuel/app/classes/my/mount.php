...
class My_Mount
{
    ...
    private function is_mount()
    {
        if (!$this->check_remote_config()) {
            // \My_Log::info('config "my"が設定されていません');
            return true;
        }
        ...
        if ($status_mount != 0) {
            \My_Log::info('mount check error - command:"' . $command_mount . '" status:' . $status_mount);
            return false;
        }
        ...
        \My_Log::info('マウントされたディレクトリが見つかりません。[' . $pattern . ']');
        return false;
    }
    ...
    private function check_mount_file()
    {
        if (!file_exists($this->mount_check_file)) {
            \My_Log::info('マウントチェックファイルが見つかりません');
            return false;
        }
        return true;
    }
    ...
    private function run_mount()
    {
        ...
        if ($this->is_mount()) {
            \My_Log::info('すでにマウントされています');
            return true;
        }

        \My_Log::info('run mount');
        $remote_path = '//' . rtrim($this->remote_host, '/') . '/' . rtrim($this->remote_path, '/');
        ...
        \My_Log::info("{$result} command:\"" . $command_mount . '" status:' . $status_mount);
    ...
    private function run_unmount()
    {
        ...
        \My_Log::info("{$result} command:\"" . $command_unmount . '" status:' . $status_unmount);
